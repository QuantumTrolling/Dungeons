...
package net.mcreator.dungens.world.features;
...
import net.mcreator.dungens.procedures.TestAdditionalGenerationConditionProcedure;
...
public class TestFeature extends Feature<NoneFeatureConfiguration> {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("dungens", "aboba"));
		if (template == null)
			return false;
		...
		if ((context.random().nextInt(1000000) + 1) <= 100000) {
			...
			for (int a = 0; a < count; a++) {
				...
				int y = spawnTo.getY();
				int z = spawnTo.getZ();
				if (!TestAdditionalGenerationConditionProcedure.execute(world, x, y, z))
					continue;
